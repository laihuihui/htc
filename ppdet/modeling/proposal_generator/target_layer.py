...
class BBoxAssigner(object):
    ...
    def __call__(self,
        ...
        outs = generate_proposal_target(
            ...
            self.cascade_iou[stage], concate_gt=concate_gt, pos_is_gts=True)
        # outs = generate_proposal_target(
        #     rpn_rois, gt_classes, gt_boxes, self.batch_size_per_im,
        #     self.fg_fraction, self.fg_thresh, self.bg_thresh, self.num_classes,
        #     self.ignore_thresh, is_crowd, self.use_random, is_cascade,
        #     self.cascade_iou[stage], delete_pos=delete_pos, pos_is_gts=pos_is_gts)
        rois = outs[0]
        rois_num = outs[4]
        ...
        # if delete_pos:
        #     rois_num = outs[-2]
        #     return rois, rois_num, targets, outs[-1]
        if pos_is_gts:
