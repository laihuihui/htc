...
class RPNHead(nn.Layer):
    ...
    def get_loss(self, pred_scores, pred_deltas, anchors, inputs):
        ...
        # a = inside_flags.astype('double').sum()
        ...
        score_tgt, bbox_tgt, loc_tgt, norm = self.rpn_target_assign(inputs,
                                                                    anchors, inside_flags = inside_flags)
        ...
        if pos_ind.shape[0] == 0:
            ...
            if self.smoothl1loss:
                beta = self.smoothl1lossbeta
                diff = paddle.abs(loc_pred - loc_tgt)
                loss_rpn_reg = paddle.where(diff < beta, 0.5 * diff * diff / beta, diff - 0.5 * beta)
